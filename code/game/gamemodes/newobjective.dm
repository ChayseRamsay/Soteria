	...
	objective
		...
		frame
			New(var/text,var/joba,var/datum/mind/targeta)
				...
				explanation_text = "Frame [target.current.real_name], the [target.assigned_role] for a crime and make sure they are arrested and brought back to the Centcom ship alive.  We'll handle the rest from there."
		...
		protection
			New(var/text,var/joba,var/datum/mind/targeta)
				...
				job = joba
				weight = get_points(job)
				explanation_text = "[target.current.real_name], the [target.assigned_role] is a [pick("relative of a","friend of a","") + pick("high ranking","important","well-liked")] Syndicate [pick("Leader","Officer","Agent","sympathiser")].  Make sure they get off the ship safely, while minimizing intervention."
		...
		steal
			...
			*/
			...
			blueprints
				...
				explanation_text = "Steal the ship's blueprints."
				weight = 20
			...
			nuke_disk
				...
				explanation_text = "Steal the ship's nuclear authentication disk."
				weight = 20
		...
		mutiny
			...
			check_completion()
				if(target && target.current)
					...
					if(target.current.stat == 2)
						return 1
					else if((T) && (T.z != 1))//If they leave the ship they count as dead for this
						return 2
					else
						return 0
				else
